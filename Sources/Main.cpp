...
const char * whatTitle() {
	if (!microFacet)
		return "Blinn & Phong BRDF";
	if (!ggx)
		return "Cook & Torrance Microfacet BRDF";
	if (schlick)
		return "GGX Microfacet BRDF with Smitch Approximation";
	else
		return "GGX Microfacet BRDF with Schlick Approximation";
}
...
void initGLFW () {
	...
	glfwSetWindowTitle(windowPtr, whatTitle());
}
...
void initScene (const std::string & meshFilename) {
	...
	GLuint toonTex = material.loadTextureFromFileToGPU(dirName + "X_toon2.png");
